...
{
	public class PricklyPearYellowPlant : ModTile
	{
        ...
        public override bool Drop(int i, int j)
        {
            int stage = Main.tile[i, j].frameX / 18 / 2;
      			...
      			if (player.HeldItem.type == ModContent.ItemType<DynastyTrowel>())
      			{
      					...
      					{
      							Item.NewItem(i * 16, j * 16, 0, 0, ModContent.ItemType<PricklyPearYellowSeed>());
      					}
      			}
            ...
            if (stage == 2 && Main.tile[i, j].frameX == 72 && Main.tile[i, j].frameY == 18)
            {
                Item.NewItem(i * 16, j * 16, 0, 0, ModContent.ItemType<PricklyPearYellow>());
            }
            return false;
        }
